...
对于**查询**：
    两个索引时间消耗没太大区别
对于**更新**过程：
    不在内存中的数据页，会将更新语句放在change buffer中，等涉及到的数据页进入内存后进行更新操作，即`merge`过程‘。
    特别是**机械硬盘**，更为推荐使用普通索引
...
唯一索引用不上`change buffer`机制，建议使用非唯一索引
redo log 主要节省的是随机写磁盘的 IO 消耗（转成顺序写），而 change buffer 主要节省的则是随机读磁盘的 IO 消耗

## 10、mysql为啥会选错索引
**优化器**用来选择索引，为用最小的代价去执行语句
**扫描行数**：
一个索引上不同的值越多，这个索引的**区分度**就越好。而一个索引上不同的值的个数，我们称之为“基数”（cardinality）
索引的基数通过**采样统计**的方法获得

优化器会预估需要扫描的表行数，计算代价。索引统计不准确就会选错索引，导致查询时间很长。可以修正：
```
analyze table t;
explain select * from table t where a between 100 and 200;
```

`force index` 可以强制选择索引
或者更改哟一下sql语义，可以起到引导优化器选择我们希望的索引的作用
...
所以，**偶尔抖一下**，可能是在**刷脏页**
    即，
    - 当InnoDB的redo log写满了的时候，
    - 系统内存不够，需要淘汰一些数据页
    - mysql认为系统空闲的时候，会进行更新数据
    - mysql正常关闭时，将内存脏页flush到硬盘上

场景二，InnoDB用buffer pool管理内存，缓冲池中的内存页有三种状态：还未使用、使用了仍是干净页、使用了是脏页

### InnoDB刷脏页策略


## 19、只查一条语句，sql为啥慢
`show processList;`查询当前语句处于什么状态
`select blocking_id from sys.schema_table_lock_waits`找到阻塞的进程，直接kill即可
